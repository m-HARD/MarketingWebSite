    ...
    <div class="mx-20">
        <div class="mb-3 bg-gray-300 grid grid-cols-12 gap-4 flex rounded-md overflow-hidden">
            ...
            <div class="col-end-13 col-span-3 py-4">
                <span class="ml-16 flex justify-center">Id</span>
            </div>
        </div>
        @foreach($products as $product)
        <div class="mb-1 bg-gray-300 grid grid-cols-12 gap-4 flex align-baseline rounded-md overflow-hidden">
            <div><img src="{{ asset('css/1.jpg') }}" alt="musab" class="h-20"></div>
            <div class="col-start-2 col-span-3 py-4 font-bold text-sm">{{ $product->name }}</div>
            <div class="col-start-5 col-span-3 py-4 font-hairline text-sm">
                {{ \Illuminate\Support\Str::limit($product->description, 65, $end='...') }}
            </div>
            <div class="py-4 text-center">${{ $product->price }}</div>
            <div class="py-4 text-center">{{ $product->stock }}</div>
            <div class="col-end-13 col-span-3 py-4">
